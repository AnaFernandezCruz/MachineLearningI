...
#dataSetValidationPCA  <-  validation_sin_naPCA  %>%  dplyr::select(c(var_modeloPCA,'SalePrice'))  %>%  na.omit()
#dataSetContinuousPCA  <-  datasetTrainPCA %>%  dplyr::select(-c('has2ndFloor'))  %>%  na.omit() 
#dataSetContinuousPCA <-  dataSetContinuousPCA  %>%    dplyr::select(columnas_continuas_PCA)   %>%  na.omit() 
...
```
  ...
  Comprobamos que el accuracy en ambos modelos es similar con un 40% de reducción en el número de variables.
