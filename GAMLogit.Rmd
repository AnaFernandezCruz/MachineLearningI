...
```
 ...
 Una vez obtenidos los modelos, los compararemos con las típicas curvas ROC (no son las mejores para la evaluación de datasets desbalanceados) y las curvas que enfrentran los parámetros precission/recall. El resultado es el que se muestra a continuación:
...
for(model_gam in model_list){
  ...
  resultParcial <- t(coords(the_roc, "all", ret = c("recall", "precision")))
...
saveRDS(modeloGAMLogisticRegressionCarosCaret_smote, "modeloGAMLogisticRegressionCarosCaret.rds")

```

   Por lo tanto, de los 4 modelos generados y atendiendo a la curva precission/recall, elegiremos el modelo  "weighted" para pasar a la fase de comparación de modelos.
