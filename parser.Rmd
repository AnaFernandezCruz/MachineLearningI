...
```{r}
library(arules)
v = discretize(dataset$SalePrice, method = "interval", breaks = 3, onlycuts = TRUE)

dataset$GrupoPrecio = cut(dataset$SalePrice,v)
levels(dataset$GrupoPrecio) = c("Barato","Normal","Caro")
...
saveRDS(datasetTrain, "datasetTrain.csv")
saveRDS(datasetTest, "datasetTest.csv")
saveRDS(datasetValidation, "datasetValidation.csv")
