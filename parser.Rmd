...
summary(vecindario_agrupado)
 ...
 anoMedio  <- round(mean(vecindario_precioGaraje$GarageYrBlt, na.rm = TRUE))
...
for(j in 1:length(vecindario_precioGaraje$GarageYrBlt)) {
  if(is.na(vecindario_precioGaraje$GarageYrBlt[j])) {
    ...
    if(length( year_a_imputar ) > 0) {
          year_corregido[length(year_corregido) + 1] <- year_a_imputar
    } else {
        year_corregido[length(year_corregido) + 1] <- anoMedio
    }
  }else {
    #year_corregido<-c(year_corregido,vecindario_precioGaraje$GarageYrBlt[j])
    year_corregido[length(year_corregido) + 1] <- vecindario_precioGaraje$GarageYrBlt[j]
    #next 
  }
  #print(paste0("Current element: ", j," y length ",length(year_corregido)))
...
dataset$MSSubClass<-revalue(dataset$MSSubClass, c('20'='1_story_1946_', '30'='1_story_1945_', '40'='1_story_unf_attic', '45'='1_5_story_unf', '50'='1_5_story_fin', '60'='2_story_1946_', '70'='2_story_1945_', '75'='2_5_story_all_ages', '80'='split_multi_level', '85'='split_foyer', '90'='duplex_all_style_age', '120'='1_story_PUD_1946_', '150'='1_5_story_PUD_all', '160'='2_story_PUD_1946', '180'='PUD_multilevel', '190'='2_family_conversion'))
...
```{r}
library(arules)
umbralParticionMonetaria = 250000
#v = discretize(dataset$SalePrice, breaks = 3, onlycuts = TRUE)
v = discretize(dataset$SalePrice, method = "fixed", breaks = c(-Inf,umbralParticionMonetaria,Inf), onlycuts = TRUE)
...
umbralParticionMonetaria = 250000
datasetTrain <- datasetTrain %>%  mutate(GrupoPrecio = ifelse(SalePrice >= log(umbralParticionMonetaria), "Caro", "Barato")) 
datasetTrain$GrupoPrecio <- as.factor(datasetTrain$GrupoPrecio)
datasetTest <- datasetTest %>%  mutate(GrupoPrecio = ifelse(SalePrice >= log(umbralParticionMonetaria), "Caro", "Barato")) 
datasetTest$GrupoPrecio <- as.factor(datasetTest$GrupoPrecio)
validation <- validation <- validation %>%  mutate(GrupoPrecio = ifelse(SalePrice >= log(umbralParticionMonetaria), "Caro", "Barato")) 
...
saveRDS(var_modelo, "var_modelo.rds")
...
write.csv(datasetTrain, "datasetTrain.csv")
write.csv(datasetTest, "datasetTest.csv")
write.csv(validation_sin_na, "datasetValidation.csv")
...
saveRDS(validation_sin_na, "datasetValidation.rds")
...
excludedVars <- c('SalePrice','GrupoPrecio','TotalSF','MoSold','YrSold')

set.seed(2018)

dataSetTrainTestBaratos <-  dplyr::union(datasetTrain,datasetTest) %>% dplyr::filter( GrupoPrecio == 'Barato' )
dataSetTrainTestCaros <-  dplyr::union(datasetTrain,datasetTest) %>% dplyr::filter( GrupoPrecio == 'Caro' )

arbol_visualizacion_baratos <- randomForest(x=dataSetTrainTestBaratos %>% dplyr::select(-excludedVars ), y=dataSetTrainTestBaratos$SalePrice, ntree=100,importance=TRUE)
varImportantesTest <- importance(arbol_visualizacion_baratos)
...
saveRDS(dataframeTestImportantesBaratos, "dataframeTestImportantesBaratos.rds")
saveRDS(arbol_visualizacion_baratos, "arbol_visualizacion_baratos.rds")

arbol_visualizacion_caros <- randomForest(x=dataSetTrainTestCaros %>% dplyr::select(-excludedVars ), y=dataSetTrainTestCaros$SalePrice, ntree=100,importance=TRUE)
varImportantesTest <- importance(arbol_visualizacion_caros)
...
saveRDS(dataframeTestImportantesCaros, "dataframeTestImportantesCaros.rds")
saveRDS(arbol_visualizacion_caros, "arbol_visualizacion_caros.rds")


numero_variables_modelo_clasificador = 20

var_modelo_clasificador =  setdiff(unique(union(dataframeTestImportantesBaratos[1:numero_variables_modelo_clasificador,1],dataframeTestImportantesCaros[1:numero_variables_modelo_clasificador,1])),var_eliminar_correlacion)

saveRDS(var_modelo_clasificador, "var_modelo_clasificador.rds")

#names(datasetTrain)[names(datasetTrain) == "Class"] <- "GrupoPrecio"  


write.csv(datasetTrain %>% dplyr::select(c(var_modelo_clasificador,c('SalePrice','GrupoPrecio')) ), "datasetTrainModeloClasificador.csv")
write.csv(datasetTest %>% dplyr::select(c(var_modelo_clasificador,c('SalePrice','GrupoPrecio')) ) , "datasetTestModeloClasificador.csv")
write.csv(validation_sin_na %>% dplyr::select(c(var_modelo_clasificador,c('SalePrice','GrupoPrecio')) ), "datasetValidationModeloClasificador.csv")

saveRDS(datasetTrain %>% dplyr::select(c(var_modelo_clasificador,c('SalePrice','GrupoPrecio')) ), "datasetTrainModeloClasificador.rds")
saveRDS(datasetTest %>% dplyr::select(c(var_modelo_clasificador,c('SalePrice','GrupoPrecio')) ), "datasetTestModeloClasificador.rds")
saveRDS(validation_sin_na %>% dplyr::select(c(var_modelo_clasificador,c('SalePrice','GrupoPrecio')) ), "datasetValidationModeloClasificador.rds")
...
splitTrainModeloClasificador <- splitmix(datasetTrain %>% dplyr::select(-c("SalePrice",'GrupoPrecio')) %>% dplyr::select(var_modelo_clasificador)) 
X1TrainModeloClasificador <- splitTrainModeloClasificador$X.quanti 
X2TrainModeloClasificador <- splitTrainModeloClasificador$X.quali 

objetoPCAMIXTrainModeloClasificador <- PCAmix(X.quanti=X1TrainModeloClasificador, X.quali=X2TrainModeloClasificador,rename.level=TRUE, graph=FALSE,ndim=numeroDimOptimo)

saveRDS(objetoPCAMIXTrainModeloClasificador, "datasetTrainModeloClasificadorPCA.rds")


splitTestModeloClasificador <- splitmix(datasetTest %>% dplyr::select(-c("SalePrice",'GrupoPrecio'))%>%  dplyr::select(var_modelo_clasificador)) 
X1TestModeloClasificador <- splitTestModeloClasificador$X.quanti 
X2TestModeloClasificador <- splitTestModeloClasificador$X.quali 
objetoPCAMIXTestModeloClasificador <- PCAmix(X.quanti=X1TestModeloClasificador, X.quali=X2TestModeloClasificador,rename.level=TRUE, graph=FALSE,ndim=numeroDimOptimo)

saveRDS(objetoPCAMIXTestModeloClasificador, "datasetTestModeloClasificadorPCA.rds")
